...
public class OTGeneratorTest {
    ...
    public void numberOfTransformations(){
       ...
       for (MarketLevel level: levels)
           numberOfTransformations+=level.getTransformations().size();
    ...
    public void goodsAreFromPreviousLevel(){
        for (int i =1; i< levels.size();i++){
            ...
            for(Transformation transformation: currentLevel.getTransformations()){
                GoodBundle bundle = transformation.getOutput().iterator().next();
                Good good = bundle.getGood();
                assertTrue("Transformation should use good from previous level", previousLevel.getGoods().contains(good));
            }
        } 
    }
