 ...
 * is added to the MinimizingCardPanel:
 * <pre>
 * myCardPanel.add(myChild, "MyChildName");
 * myChild.getName() <i>=> "MyChildName"</i>
 * </pre>
 * As with CardLayout, the first child added to a MinimizingCardPanel is made visible
 ...
 * </pre>
 * <p>
 * The MinimizingCardPanel class doesn't support the vgap/hgap CardLayout properties since
 * one can add a Border, see JComponent.setBorder().
 */

public class MinimizingCardPanel extends JPanel
{
    private static class Layout implements LayoutManager
    {
	/** 
	 * Set the childs name (if non-null) and and make it visible 
	 * iff it's the only MinimizingCardPanel child.
	...
	/**
	 * @return visible child preferred size
	 */
	public Dimension preferredLayoutSize(Container parent) {

        return visibleChild(parent).getPreferredSize();

	}

        /**
	 * @return visible child minimum size
	 */
	public Dimension minimumLayoutSize(Container parent) {

        return visibleChild(parent).getMinimumSize();
	}

    private Component visibleChild(Container parent) {
        Component visibleChild=parent.getComponent(0);
        for(Component child: parent.getComponents()){
            if (child.isVisible()) visibleChild=child;
        }
        return visibleChild;
    }
    ...
    /**
     * Creates a MinimizingCardPanel.  Children, called "cards" in this API, should be added
    ...
    /**
     * Show the card that was added to this MinimizingCardPanel after the currently
    ...
    /**
     * Show the card that was added to this MinimizingCardPanel before the currently
    ...
    /**
     * Show the first card that was added to this MinimizingCardPanel.
     */
    ...
    /**
     * Show the last card that was added to this MinimizingCardPanel.
     */
